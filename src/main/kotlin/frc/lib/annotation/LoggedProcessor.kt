...
// Blatantly stole from https://github.com/Team-8-bit/robot-lib/blob/eeca0adc0aef9ddebdf0f7ffa90fb2f635fd1ba2/annotation/src/main/kotlin/org/team9432/annotation/LoggedProcessor.kt
...
class LoggedProcessor(private val codeGenerator: CodeGenerator): SymbolProcessor {
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        val annotatedClasses = resolver.getSymbolsWithAnnotation("frc.lib.annotation.Logged").filterIsInstance<KSClassDeclaration>()
    ...
    private fun process(classDeclaration: KSClassDeclaration) {
        ...
        // val file = FileSpec.builder(packageName, newClassName).addType(type.build()).indent("    ").addImport("frc.lib.advantagekit", listOf("kGet", "kPut")).build()
